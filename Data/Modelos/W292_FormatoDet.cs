...
public class W292_FormatoDet
{
    [Key]
    [StringLength(65)]
    public string FormatoDetId { get; set; } = MyFunc.MyGuid("FDet");
    ...
    public W292_FormatoDet(string formatoId, int orden, string tipo, 
        string campo, string descripcion, int estado, bool status)
    {
        FormatoId = formatoId;  
        Orden = orden;
        Tipo = tipo;
